...
//Turn Each LED on one after another
  for(int i = 0; i <= 7; i++){
    ...
    //gets one added to it so this will repeat zz
...
void oneOnAtATime(){
  ...
  for(int i = 0; i <= 7; i++){
    int offLED = i - 2;  //Calculate which LED was turned on last time through
 ...
 */
void inAndOut(){
  int delayTime = 10; //the time (in milliseconds) to pause between LEDs
                       //make smaller for quicker switching and larger for slower
